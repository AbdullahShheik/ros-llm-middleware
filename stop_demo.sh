...
# mops up any ROS/Gazebo processes it left behind (e.g. if a pane was
# killed but a child process detached from tmux's process tree).
#
# Usage: ./stop_demo.sh
...
# Belt-and-suspenders: some launched processes (gz sim, move_group, node
# spawners) can outlive their pane if they were backgrounded oddly. Look
# for the demo's known process patterns and offer to clean them up.
PATTERNS=(
  "ros2 launch world world.launch.py"
  "ros2 launch actuator actuator.launch.py"
  "ros2 run perception perception_node.py"
  "ros2 run action_dispatcher ik_feasibility_service.py"
  "ros2 run action_dispatcher dispatcher_node.py"
